...
namespace Elite
{
	...
	inline vector<T_NodeType*> EulerianPath<T_NodeType, T_ConnectionType>::FindPath(Eulerianity& eulerianity) const
	{
		...
		int currNodeIdx = invalid_node_index;
		...
		{
			// Find First Odd One
			std::vector<GraphNode2D*> activeNodes = m_pGraph->GetAllActiveNodes();
			...
			for (auto activeNode : activeNodes)
			{
				...
				{
					currNodeIdx = activeNode->GetIndex();
					break;
				}
			}
		}
		...
		do
		{
			if (graphCopy->GetNodeConnections(graphCopy->GetNode(currNodeIdx)).size()) 
			{
				nodeStack.push(currNodeIdx);
				int neighborIndex = graphCopy->GetNodeConnections(graphCopy->GetNode(currNodeIdx)).front()->GetTo();
				graphCopy->RemoveConnection(nodeStack.top(), neighborIndex);
				currNodeIdx = neighborIndex;
			}
			else 
			{
				path.push_back(graphCopy->GetNode(currNodeIdx));
				int lastId = nodeStack.top();
				nodeStack.pop();
				currNodeIdx = lastId;
			}
			
		} 
		while (graphCopy->GetNodeConnections(graphCopy->GetNode(currNodeIdx)).size() && !nodeStack.size());
