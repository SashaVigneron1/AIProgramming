...
#pragma region Rect
	...
	inline bool IsOverlapping(EPhysicsCircleShape circle, Elite::Rect rect)
	{
		float distanceToCircleX = abs(circle.position.x - (rect.bottomLeft.x + rect.width / 2.f));
		float distanceToCircleY = abs(circle.position.y - (rect.bottomLeft.y + rect.height / 2.f));

		if (distanceToCircleX > (rect.width / 2 + circle.radius)) { return false; }
		if (distanceToCircleY > (rect.height / 2 + circle.radius)) { return false; }

		if (distanceToCircleX <= (rect.width / 2)) { return true; }
		if (distanceToCircleY <= (rect.height / 2)) { return true; }

		float distanceToCorner = (distanceToCircleX - rect.width / 2) * (distanceToCircleX - rect.width / 2) +
			(distanceToCircleY - rect.height / 2) * (distanceToCircleY - rect.height / 2);

		return (distanceToCorner <= (circle.radius * circle.radius));
	}
