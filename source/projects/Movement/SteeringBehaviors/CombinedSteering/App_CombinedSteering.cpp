...
{	
	...
	SAFE_DELETE(m_pPriorritySteering);
	SAFE_DELETE(m_pSoberWander);
	SAFE_DELETE(m_pSoberFlee);
}
...
void App_CombinedSteering::Start()
{
	...
	weightedSteeringBehaviors.push_back({ m_pDrunkSeek, 0.5f });
	...
	// Priority Steering
	m_pSoberFlee = new Flee();
	m_pSoberWander = new Wander();
	m_pSoberWander->SetWanderOffset(0.f);

	m_pPriorritySteering = new PrioritySteering({ m_pSoberFlee, m_pSoberWander });

	m_pSoberAgent = new SteeringAgent();
	m_pSoberAgent->SetSteeringBehavior(m_pPriorritySteering);
	m_pSoberAgent->SetBodyColor(Color(0, 0, 1));
...
void App_CombinedSteering::Update(float deltaTime)
{
	...
	m_pSoberFlee->SetTarget(TargetData(m_pDrunkAgent->GetPosition()));
